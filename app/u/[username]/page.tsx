...
export default function ProfilePage() {
  ...
  /* Deep link from a follow_request notification: /u/me?open=requests opens
     and scrolls to the accept/decline panel on your own profile. */
  useEffect(() => {
    if (!isMe) return;
    if (new URLSearchParams(window.location.search).get('open') !== 'requests') return;
    const t = setTimeout(() => {
      requestsRef.current?.setAttribute('open', '');
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      setNotFound(false);
      /* viewAsVisitor is in the dep array below: toggling it re-runs the load
         so the owner sees exactly what a signed-out/other user would see. */
      ...
      /* /u/me deep-links to the signed-in user's own profile — but only as
         a FALLBACK, so a user actually named "me" stays reachable */
      let handle = username;
      let { data: profileData } = await supabase
        ...
        .eq('username', handle)
        .maybeSingle();
      if (!profileData && handle === 'me') {
        const { data: meRow } = await supabase
          .from('profiles')
          .select('username')
          .eq('id', user.id)
          .maybeSingle();
        handle = (meRow?.username as string) || '';
        if (handle) {
      ...
      if (me.id !== user.id) {
        ...
        const [{ data: followRow }, { data: followReq }, { data: theyFollowMe }] = await Promise.all([
        ...
        setTheyFollowMe(!!theyFollowMe);
      }

      /* Their public + followers-only (if following) posts. Visitor preview
         (own profile, viewAsVisitor) sees exactly what a stranger would:
         public/link posts only — private and followers-only are hidden. */
      ...
      if (me.id === user.id && viewAsVisitor) {
        list = list.filter((p) => p.visibility === 'public' || p.visibility === 'link');
      }
      ...
      /* Journals: owner sees all of theirs; everyone else (including the
         owner in visitor preview) only sees public/link journals. */
      if (me.id === user.id && !viewAsVisitor) {
  ...
  const startChat = async () => {
    ...
    if (error || !conv) {
      void alert({ title: 'Could not start chat', message: error?.message || 'Please try again.', tone: 'error' });
      return;
    }
    ...
    <main className="min-h-[100dvh] bg-[#FFF7F8] px-3 pb-24 pt-5 text-[#111111] sm:px-6 md:pb-10">
      <div className="mx-auto w-full max-w-2xl">
        ...
        {/* Own profile → incoming follow requests (private accounts).
            Hidden in visitor preview — a stranger never sees these. */}
        {isMe && !viewAsVisitor && (
          <details
            ref={requestsRef}
            id="follow-requests"
            className="mt-4 rounded-2xl border border-[#E8E2E4] bg-white p-4 shadow-sm"
          >
        ...
        {activeTab === 'journals' && (
          <section className="mt-4">
            {journals.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[#E8E2E4] bg-white/60 px-6 py-10 text-center text-sm text-[#6B6B6B]">
                {isMe && !viewAsVisitor ? 'Create your first journal to see it here.' : 'No public journals yet.'}
              </div>
            ) : (
