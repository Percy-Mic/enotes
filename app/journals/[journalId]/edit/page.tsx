...
export default function JournalCanvasStudio() {
  ...
  /* ---- drag / resize: one pointer path for mouse, touch and pen ----
     touch-action:none on the wrapper and handles stops the browser from
     scrolling mid-gesture; setPointerCapture keeps the gesture alive when
     the finger leaves the page; pointercancel cleans everything up. */
  const beginGesture = (e: React.PointerEvent<HTMLElement>, el: CanvasElement, mode: 'drag' | 'resize') => {
      ...
      mutateActiveElements(
        (els) =>
          els.map((item) =>
            item.id === el.id
              ? mode === 'drag'
                ? { ...item, position_x: bounds.x, position_y: bounds.y }
                : { ...item, width: size.w, height: size.h }
              : item
          ),
        mode === 'drag' ? 'Move element' : 'Resize element',
        `${mode}-${el.id}`
      );
    const onMove = (ev: PointerEvent) => {
      if (mode === 'drag') {
        bounds.x = Math.max(0, Math.min(BASE_WIDTH - el.width, bounds.x + (ev.clientX - last.x) / canvasScale));
        bounds.y = Math.max(0, Math.min(BASE_HEIGHT - el.height, bounds.y + (ev.clientY - last.y) / canvasScale));
      } else {
        size.w = Math.max(50, Math.min(BASE_WIDTH - el.position_x, size.w + (ev.clientX - last.x) / canvasScale));
        size.h = Math.max(50, Math.min(BASE_HEIGHT - el.position_y, size.h + (ev.clientY - last.y) / canvasScale));
      }
      last.x = ev.clientX;
      last.y = ev.clientY;
      apply();
  ...
  const startDrag = (e: React.PointerEvent<HTMLElement>, el: CanvasElement) => beginGesture(e, el, 'drag');
  const startResize = (e: React.PointerEvent<HTMLElement>, el: CanvasElement) => beginGesture(e, el, 'resize');
  ...
  const renderElement = (el: CanvasElement, interactive = true) => {
    ...
    /* Selection chrome lives inside the zoomed page, so it is sized in
       inverse-zoom units: it renders at a constant size ON SCREEN — a
       finger-sized target at any zoom level, on any device. */
    const inv = interactive ? 1 / Math.max(zoom, 0.05) : 1;
    const style: React.CSSProperties = {
      ...
      boxShadow: selected ? `0 0 0 ${3 * inv}px rgba(255,255,255,.9), 0 0 0 ${5.5 * inv}px #ec4899` : undefined,
    };
    return (
      <div
        key={el.id}
        ...
        className={`absolute ${interactive ? 'cursor-move touch-none' : ''}`}
        style={style}
      >
        ...
        {el.type === 'media' && el.media_url && (
          <div className="h-full w-full overflow-hidden bg-white/80" style={{ borderRadius: el.border_radius ?? 12 }}>
            {el.media_type === 'video' ? (
              <video onPointerDown={(e) => e.stopPropagation()} src={el.media_url} controls playsInline className="h-full w-full" style={{ objectFit: el.object_fit }} />
            ) : el.media_type === 'audio' ? (
              <div className="flex h-full flex-col justify-center gap-2 p-4">
                <div className="flex items-center gap-2 text-sm font-semibold"><FileAudio className="h-5 w-5" /> {el.content}</div>
                <audio onPointerDown={(e) => e.stopPropagation()} src={el.media_url} controls className="w-full" />
              </div>
            ) : (
        ...
        {el.type === 'link' && (
          <a href={el.href || el.content} target="_blank" rel="noreferrer" onPointerDown={(e) => e.stopPropagation()} onClick={(e) => interactive && e.preventDefault()} className="flex h-full w-full items-center gap-3 border border-blue-200 bg-blue-50/80 p-4 text-blue-800 shadow-sm">
        ...
        {el.type === 'drawing' && (
          <div onPointerDown={(e) => { e.stopPropagation(); setSelectedElementId(el.id); }} className="h-full w-full overflow-hidden bg-transparent">
        ...
        {selected && (
          <>
            <div onPointerDown={(e) => startDrag(e, el)} className="absolute left-1/2 top-1 z-30 flex -translate-x-1/2 cursor-move touch-none items-center rounded-full border border-white/80 bg-[#2A211D]/85 font-bold text-white shadow" style={{ minHeight: 30 * inv, paddingTop: 4 * inv, paddingBottom: 4 * inv, paddingLeft: 12 * inv, paddingRight: 12 * inv, fontSize: 11 * inv, borderWidth: Math.max(1, inv) }}>⠿ drag</div>
            ...
            {confirmDeleteId === el.id ? (
              <span className="absolute right-0 z-40 flex items-center bg-red-600 shadow-lg" style={{ top: -52 * inv, gap: 4 * inv, padding: 4 * inv, borderRadius: 12 * inv }}>
                <button
                  type="button"
                  onPointerDown={(e) => e.stopPropagation()}
                  onClick={(e) => { e.stopPropagation(); deleteSelected(); setConfirmDeleteId(null); }}
                  className="rounded-lg bg-white/15 font-bold text-white"
                  style={{ minHeight: 36 * inv, paddingInline: 12 * inv, fontSize: 12 * inv }}
                >
                  Confirm
                </button>
                <button
                  ...
                  style={{ width: 36 * inv, height: 36 * inv }}
                  aria-label="Cancel delete"
                >
                  <X style={{ width: 16 * inv, height: 16 * inv }} />
                </button>
              </span>
            ) : (
              <button
                ...
                className="absolute right-0 z-40 flex items-center justify-center rounded-full bg-white text-red-500 shadow transition hover:ring-2 hover:ring-red-300"
                style={{ top: -48 * inv, width: 36 * inv, height: 36 * inv }}
                aria-label="Delete element (tap again to confirm)"
                title="Delete element"
              >
                <Trash2 style={{ width: 17 * inv, height: 17 * inv }} />
              </button>
            )}
            <div onPointerDown={(e) => startResize(e, el)} className="absolute flex cursor-se-resize touch-none items-center justify-center rounded-full bg-pink-500 shadow ring-2 ring-white" style={{ right: -14 * inv, bottom: -14 * inv, width: 36 * inv, height: 36 * inv }}><Maximize2 style={{ width: 16 * inv, height: 16 * inv }} className="text-white" /></div>
          </>
        )}
      </div>
    );
    ...
    <main className="min-h-[100dvh] overflow-x-hidden bg-[#B88C5A] pb-24 text-[#241F1B] xl:pb-6" style={{ backgroundImage: 'repeating-linear-gradient(0deg,rgba(255,255,255,.035) 0,rgba(255,255,255,.035) 1px,transparent 1px,transparent 6px),repeating-linear-gradient(90deg,rgba(70,40,15,.025) 0,rgba(70,40,15,.025) 2px,transparent 2px,transparent 13px)' }}>
      ...
      <div className="mx-auto grid max-w-[1500px] grid-cols-1 gap-4 p-3 pt-[76px] md:p-5 md:pt-[92px] xl:grid-cols-[260px_minmax(0,1fr)_290px]">
        ...
        <section className={`${showCanvas ? '' : 'hidden'} min-w-0 rounded-2xl border border-[#D8C9BA] bg-[#8A6039] p-2 shadow-[inset_0_0_0_2px_rgba(60,35,18,.25)] md:p-4 xl:order-2`}>
          ...
          {/* STAGE */}
          <div className="relative flex items-start justify-center overflow-auto rounded-xl border-2 border-[#68472D] bg-[#8A6039] p-3 md:p-6" style={{ minHeight: isMobile ? `min(62dvh, ${Math.round(BASE_HEIGHT * zoom) + 48}px)` : `min(72vh, ${Math.round(BASE_HEIGHT * zoom) + 60}px)` }} onPointerDown={() => { if (tool === 'select') setSelectedElementId(null); }}>
