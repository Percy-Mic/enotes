...
export default function CommunitiesPage() {
  ...
  const [items, setItems] = useState<
    { id: string; slug: string; name: string; description: string; topic: string; emoji: string; members_count: number; posts_count: number; icon_url?: string | null }[]
  >([]);
  ...
  const load = useCallback(async () => {
    ...
    const query = supabase
      .from('communities')
      .select('id, slug, name, description, topic, emoji, members_count, posts_count, icon_url')
    ...
    /* icon_url ships with migration 2026-09-19 — tolerate a not-yet-migrated
       database by retrying without the column (badges fall back to emoji). */
    ...
    if (!primary.data) {
      ...
      if (filter.trim()) {
        const safe = filter.trim().replace(/[%_,]/g, '');
        retry.or(`name.ilike.%${safe}%,topic.ilike.%${safe}%,description.ilike.%${safe}%`);
      }
      const fallback = await retry;
      rows = (fallback.data || []) as Record<string, unknown>[];
    }
    setItems(rows.map((c) => ({ ...c, icon_url: (c.icon_url as string | null) ?? null })) as typeof items);
            ...
            {items.map((c) => (
              ...
              >
                <Link href={`/communities/${c.slug}`} className="flex min-w-0 items-start gap-3">
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[#FFF7F8] text-2xl">
                    {c.icon_url ? (
