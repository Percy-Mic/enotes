...
import { Bell, BellOff, BellRing, CheckCheck, Heart, Loader2, MessageCircle, Palette, PhoneMissed, Radio, Share2, Sparkles, UserCheck, UserPlus, Users } from 'lucide-react';
...
const ICONS: Record<string, React.ReactNode> = {
  ...
  follow_request: <UserPlus className="h-4 w-4 text-[#8B5CF6]" />,
  follow_accept: <UserCheck className="h-4 w-4 text-[#10B981]" />,
  share: <Share2 className="h-4 w-4 text-[#10B981]" />,
  message: <MessageCircle className="h-4 w-4 text-[#F59E0B]" />,
  chat_theme: <Palette className="h-4 w-4 text-[#E5798F]" />,
...
function target(n: Notification): string {
  switch (n.type) {
    ...
    case 'follow_request':
      /* accept/decline lives on the RECIPIENT's own profile — linking to the
         requester would dead-end the action */
      return '/u/me?open=requests';
    case 'share':
    case 'journal_invite':
      ...
      <div className="mx-auto w-full max-w-2xl">
        <header className="mb-4 flex items-center justify-between">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            {/* Gate on the mounted-state, never on pushSupported() — reading
                window during render makes the server HTML differ from the
                client's and kills hydration. 'unsupported' is only ever
                resolved post-mount by getPushState(). */}
