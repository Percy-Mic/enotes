  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('full_text_name, username, avatar_url, bio, is_private')
        .eq('id', user.id)
        .maybeSingle();
    ...
    <main className="min-h-[100dvh] bg-[#FFF7F8] px-3 pb-24 pt-5 text-[#111111] sm:px-6 md:pb-10">
      <div className="mx-auto w-full max-w-xl">
        ...
        <form onSubmit={saveProfile} className="space-y-5 rounded-2xl border border-[#E8E2E4] bg-white p-5 shadow-sm sm:p-6">
          ...
          {/* Private account — follow requests instead of instant follows */}
          <label htmlFor="isPrivate" className="flex cursor-pointer items-start justify-between gap-4 rounded-lg border border-[#E8E2E4] p-4">
            <span>
              <span className="block text-sm font-semibold">Private account</span>
              <span className="mt-0.5 block text-xs text-[#6B6B6B]">
                Only approved followers can see your followers-only posts. New followers must send a request you accept or decline.
              </span>
            </span>
            <input
              id="isPrivate"
