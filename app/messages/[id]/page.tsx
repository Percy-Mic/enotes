...
import {
  ArrowLeft, Ban, BellOff, BellRing, Camera, Check, Crown, Flag, Loader2, LogOut,
...
function ChatRoom() {
    ...
    (async () => {
      ...
      const [{ data: conv }, { data: memberRows }, { data: theme }, { data: mySettings }] = await Promise.all([
        ...
        supabase
          ...
          .eq('conversation_id', conversationId)
          .neq('user_id', user.id),
        supabase.from('chat_themes').select('*').eq('conversation_id', conversationId).eq('user_id', user.id).maybeSingle(),
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'message_reactions' }, async () => {
        ...
        /* Merge DB truth for EVERY loaded message: update messages whose
           reactions changed AND clear ones whose last reaction was removed —
           an update-only merge kept stale pills alive after unlike. */
        setMessages((list) =>
          list.map((m) => {
  ...
  const handleForward = useCallback(
    async (message: Message) => {
      /* simple forward: re-send content into the same chat with attribution */
      await handleSend({
