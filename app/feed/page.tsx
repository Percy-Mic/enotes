...
export default function FeedPage() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    let repostRows: { post_id: string; user_id: string; username: string | null }[] = [];

    if (postIds.length) {
      const [{ data: likes }, { data: counts }, { data: rps }] = await Promise.all([
      ...
      /* PostgREST returns the embedded profile as an OBJECT ({ username: … }),
         ...
         Object]" and crashes the whole feed (dir-dynamic-href error). */
      repostRows = ((rps || []) as unknown as {
        post_id: string;
        user_id: string;
      ...
      commentCounts = (counts || []).reduce((acc: Record<string, number>, row: any) => {
    ...
    list = list.map((p) => ({
      ...
      reposted_by:
        repostRows.find((r) => r.post_id === p.id && r.user_id === myId)?.username ||
