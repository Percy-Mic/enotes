...
export default function TemplateReviewDashboard({
  ...
  /* FULLSCREEN REVIEW — whole-screen preview with the approve/reject
     actions on top of it, so the decision is made looking at the actual
     video, not a 380px thumbnail. Esc exits; Space toggles playback. */
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fc] text-slate-950">
                        ...
                        {selectedTemplate.preview_url && (
                          <div className="flex shrink-0 gap-2">
                            <button
                              ...
                              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-700"
                            >
                              <Maximize2 size={14} />
                              Fullscreen review
                            </button>
      ...
      {showRejectDialog &&
        ...
        {/* FULLSCREEN REVIEW OVERLAY — whole-screen video + the decision
            buttons on top; approve/reject reuse the same handlers as the
            side panel, so state, history, and toasts stay in sync. */}
        {fullscreen && selectedTemplate && (
          <div
            className="fixed inset-0 z-[80] flex flex-col bg-black"
            role="dialog"
            aria-label="Fullscreen template review"
          >
            <div className="relative flex flex-1 items-center justify-center overflow-hidden">
              {selectedTemplate.preview_url ? (
                <video
                  ...
                  poster={selectedTemplate.thumbnail_url || undefined}
                  controls
                  autoPlay
                  loop
                  playsInline
              ...
              {/* status + exit, floating over the video */}
              <div className="absolute inset-x-0 top-0 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="max-w-[50vw] truncate text-sm font-semibold text-white/90">
                    {selectedTemplate.title}
                  </span>
                </div>
                <button
                  onClick={() => setFullscreen(false)}
                  aria-label="Exit fullscreen review"
            ...
            {/* decision bar — big, obvious, same handlers as the panel */}
            <div className="relative z-10 border-t border-white/10 bg-black/95 px-4 py-4">
              <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-3">
                {selectedTemplate.status === "pending" || selectedTemplate.status === "draft" ? (
                  <>
                    <button
                      onClick={() => {
                      ...
                      className="inline-flex min-h-[52px] flex-1 items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-8 text-base font-bold text-white transition hover:bg-emerald-400 disabled:opacity-50 sm:flex-none"
                    >
                    ...
                    <button
                      ...
                      disabled={busyAction !== null}
                      className="inline-flex min-h-[52px] flex-1 items-center justify-center gap-2 rounded-2xl bg-red-500/90 px-8 text-base font-bold text-white transition hover:bg-red-500 disabled:opacity-50 sm:flex-none"
                    >
                      <XCircle size={20} /> Reject…
                    </button>
                    <button
                      onClick={() => fsVideoRef.current?.play().catch(() => undefined)}
                      aria-label="Replay preview"
                      className="inline-flex h-[52px] w-[52px] items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-white/25"
                    >
                      <RotateCcw size={18} />
                    </button>
                  </>
                  ...
                  <p className="text-sm text-white/60">
                    This template is {statusLabel(selectedTemplate.status).toLowerCase()} — actions are in the side panel.
                  </p>
                )}
              </div>
              <p className="mt-2 text-center text-[11px] text-white/40">
                Space play/pause · Esc to exit · decision applies instantly
              </p>
            </div>
          </div>
        )}
