...
import {
  ...
  Upload, Users, VolumeX, Volume2, X, Save, Share2,
} from 'lucide-react';
...
import {
  CANVAS_SIZES, DEFAULT_ADJUSTMENTS, DEFAULT_AUDIO_PROCESSING, DEFAULT_TRANSFORM, EFFECT_PRESETS, FILTER_PRESETS, SPEED_OPTIONS,
  ...
  placeholderSrc, projectDuration, removeTimelineTrack, resolveTime, sanitizeCrop,
  type AspectRatio, type AudioTrack, type CropRect, type TimelineElement, type VideoClip, type VideoProject,
} from '@/lib/video/project';
import {
  EXPORT_QUALITY_PRESETS, VideoRenderer, defaultExportSettings,
...
function VideoEditor() {
  ...
  const addTextElement = () => {
    const el: TimelineElement = {
      ...
      start: playheadRef.current, end: Math.min(duration, playheadRef.current + 3),
  ...
  const addGifElement = (gifUrl: string) => {
    const el: TimelineElement = {
      id: makeVideoId('el'), kind: 'image', content: 'gif', src: gifUrl, track_id: project.tracks[0]?.id,
      start: playheadRef.current, end: Math.min(duration, playheadRef.current + 3),
      x: project.canvas.width / 2 - 120, y: project.canvas.height / 2 - 120,
      width: 240, height: 240, rotation: 0, opacity: 1, z: project.elements.length + 1,
      animation: 'fade',
    };
  ...
  const saveAsTemplate = async () => {
    ...
    setTplOpen(false);
    notify('Template submitted for review. 🎉');
  };
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        ...
        <section ref={stageRef} className="shrink-0 px-3 pt-2">
          ...
          <div className="flex items-center justify-center gap-3 py-1.5">
            ...
            <button onClick={() => setPlaying((p) => !p)} aria-label={playing ? 'Pause' : 'Play'} className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-black focus-visible:ring-2 focus-visible:ring-[#FFB6C1]">
                        ...
                        return (
                          ...
                          >
                            <span className="pointer-events-none truncate">
                              {el.kind === 'text' ? `T ${el.content}` : el.kind === 'sticker' ? el.content.slice(0, 3) : el.kind === 'video' ? '🎬 video' : el.kind === 'gif' ? 'GIF' : el.kind === 'shape' ? '◇' : '🖼 image'}
                            </span>
                            <span
                              data-timeline-handle="true"
      ...
      <section className="min-h-[120px] shrink-0 basis-[30dvh] overflow-y-auto overscroll-contain border-t border-white/10 px-3 py-3">
        {tool === 'media' && (
          <div className="space-y-3">
            ...
            <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border-2 border-dashed border-white/20 py-6 text-sm font-semibold text-white/70">
              <Upload className="h-5 w-5" /> Import video or photo
              <input
                type="file"
                accept="video/*,image/*"
                multiple
        ...
        {tool === 'stickers' && (
          <div className="space-y-3">
            ...
            <div className="rounded-xl bg-white/5 p-2">
              <p className="mb-1 px-1 text-xs font-semibold text-white/60">GIFs</p>
              <GifPicker onPick={(gif) => addGifElement(gif.url)} />
            </div>
...
function ElementInspector({ el, duration, onChange, onDuplicate, onDelete, onCrop }: {
  el: TimelineElement;
  duration: number;
