...
/**
 * /users/[userId]/following — everyone this profile follows.
 * Every entry shows a live Follow/Following button: for your own list that's
 * the "Following → Unfollow" action (immediate, optimistic, DB-backed).
 */
export default function FollowingPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { users: first, hasMore: more } = await fetchUserList(userId, 'following', 0);
      setUsers(first);
      setHasMore(more);
      setPage(0);
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    const next = page + 1;
    const { users: more, hasMore: moreAfter } = await fetchUserList(owner.id, 'following', next);
      ...
      <div className="mx-auto w-full max-w-xl">
        <header className="mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#6B6B6B]">People {name} follows</p>
          </div>
        </header>
        ...
        {!userId || (!loading && !owner) ? (
          ...
          <>
            <UserList
              ...
              emptyBody={
                owner && myId === owner.id
                  ? 'You can follow writers from search or their profiles — they will show up here.'
