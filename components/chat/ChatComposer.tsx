...
export default function ChatComposer({ conversationId, myId, replyingTo, onCancelReply, onSend, onTyping }: ChatComposerProps) {
  ...
  /* Escape closes the open picker (desktop); on touch, the full-screen
     scrim rendered below handles tap-to-close. */
  useEffect(() => {
  ...
  const doSend = async () => {
    ...
    if (!draft.trim() && !file && !voiceBlob && !staged) return;
    ...
    try {
      ...
      if (voiceSnapshot) {
        ...
        payload = {
          ...payload,
          content: staged.kind === 'sticker' ? staged.value : staged.description || 'GIF',
          message_type: staged.kind,
          media_url: staged.kind === 'gif' ? staged.value : null,
          media_type: staged.kind === 'gif' ? 'image/gif' : null,
        };
      }
  ...
  /* Stickers and GIFs used to auto-send on pick — one tap fired an outbound
     message with no confirmation, an easy misfire on touch screens. Both now
     stage as a draft preview with an explicit send button. */
  const [staged, setStaged] = useState<{ kind: 'sticker' | 'gif'; value: string; preview: string; description: string | null } | null>(null);

  const stageSticker = (value: string) => {
    setPanel('none');
    setStaged({ kind: 'sticker', value, preview: value, description: null });
  };

  const stageGif = (gif: GifItem) => {
    setPanel('none');
    setStaged({ kind: 'gif', value: gif.url, preview: gif.preview || gif.url, description: gif.description || null });
  };
  ...
  const sendStaged = async () => {
    ...
    setStaged(null);
    await onSend({
      content: s.kind === 'sticker' ? s.value : s.description || 'GIF',
      message_type: s.kind,
      media_url: s.kind === 'gif' ? s.value : null,
      media_type: s.kind === 'gif' ? 'image/gif' : null,
      media_name: null,
  ...
  return (
    ...
    >
      {/* Tap-to-close scrim behind any open picker — on phones this is the
          natural "tap anywhere to dismiss" affordance the tiny trigger
          button could never provide. */}
      {panel !== 'none' && (
        <button
          type="button"
          tabIndex={-1}
      ...
      {/* Staged sticker/GIF preview — nothing is sent until Send is tapped */}
      {staged && (
        <div className="mx-auto mb-2 flex max-w-2xl items-center gap-3 rounded-xl border border-[#E8E2E4] bg-white p-2">
          {staged.kind === 'gif' ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={staged.preview} alt={staged.description || 'GIF'} className="h-14 w-14 rounded-lg object-cover" />
          ) : (
            <span className="flex h-14 w-14 items-center justify-center rounded-lg bg-[#FFF7F8] text-3xl">{staged.preview}</span>
          )}
          <p className="min-w-0 flex-1 truncate text-xs text-[#6B6B6B]">{staged.kind === 'gif' ? staged.description || 'GIF' : 'Sticker'} — ready to send</p>
          <button type="button" onClick={() => setStaged(null)} aria-label="Remove sticker or GIF" className="flex h-9 w-9 items-center justify-center rounded-full hover:bg-gray-100">
            <X className="h-4 w-4" />
          </button>
          <button type="button" onClick={() => void sendStaged()} disabled={sending} className="flex h-9 items-center rounded-full bg-black px-4 text-xs font-bold text-[#FFB6C1] disabled:opacity-50">
            Send
          </button>
        </div>
      )}
      ...
      <div className="mx-auto flex w-full max-w-2xl items-end gap-1.5 max-sm:flex-col-reverse max-sm:items-stretch max-sm:gap-1">
        ...
        <div className="flex min-w-0 max-sm:items-center max-sm:gap-1 max-sm:overflow-x-auto max-sm:py-0.5">
          ...
          {panel === 'emoji' && (
            <div className="picker-sheet relative z-40">
          ...
          {panel === 'sticker' && (
            <div className="picker-sheet relative z-40">
              <StickerPicker onPick={stageSticker} />
            </div>
          )}
        </div>
