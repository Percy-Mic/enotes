...
/**
 * Shared conversation theme — every member sees the same look. Saving
 * upserts the single (conversation_id)-keyed chat_themes row, which fires
 * the chat_theme_notify DB trigger: the other members receive both a
 * realtime chat_themes UPDATE (chat UI restyles instantly) and a
 * notification ("Alex changed the chat theme").
 */
export default function ChatThemePicker({ conversationId, myId, current, onClose, onSaved }: ChatThemePickerProps) {
  ...
  const save = async (theme: Partial<ChatThemeRow> | null) => {
    setSaving(true);
    ...
    if (theme) {
      const payload = {
        ...
        background_url: theme.background_url ?? null,
      ...
      const res = await supabase
        ...
        .maybeSingle();
      error = res.error;
      if (!error) onSaved((res.data as ChatThemeRow) || null);
    } else {
      const res = await supabase
        .from('chat_themes')
        .delete()
    ...
    <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/40 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose} role="dialog" aria-modal="true" aria-label="Chat theme">
      <div className="max-h-[85dvh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mb-3 text-xs text-[#6B6B6B]">
          Everyone in this chat sees the same theme — changing it restyles it for all members.
        </p>
