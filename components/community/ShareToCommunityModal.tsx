...
export default function ShareToCommunityModal({
  ...
  const share = async () => {
    ...
    /* note needs migration 2026-09-19_community_share_note — retry without
       it so sharing still works on a not-yet-migrated database. */    let err: { message: string } | null = null;
    if (trimmed) {
      ({ error: err } = await supabase
        .from('community_posts')
        .insert({ community_id: communityId, post_id: selected, posted_by: myId, note: trimmed }));
