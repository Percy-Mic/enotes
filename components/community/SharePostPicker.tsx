...
export default function SharePostPicker({
  ...
  const [communities, setCommunities] = useState<
    { id: string; slug: string; name: string; emoji: string; members_count: number }[]
  >([]);
                ...
                <ul className="space-y-2">
                  {communities.map((c) => (
                    <li key={c.id}>
                      ...
                      >
                        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFF7F8] text-xl">
                          {c.emoji}
                        </span>
