...
export default function AppNav() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (active && profile) {
        ...
        setIsAdmin(!!(profile as { is_admin?: boolean }).is_admin);
      }
    })();
  ...
  return (
    <>
      {/* Desktop top bar — one fixed-height row: nav links left-aligned,
          Post + Admin (identity actions) anchored right. Every control is
          the same height and shares one vertical center line. */}
        ...
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-1 px-6">
          {TABS.map((tab) => {
            ...
            return (
              <Link
                key={tab.href}
                href={href}
                className={`relative flex h-10 items-center gap-1.5 rounded-xl px-3.5 text-sm font-medium transition ${
                  active ? 'bg-black/5 text-[#111111]' : 'text-[#6B6B6B] hover:bg-black/5 hover:text-[#111111]'
                }`}
              >
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link
              ...
              className="flex h-10 items-center gap-1.5 rounded-xl bg-black px-4 text-sm font-semibold text-[#FFB6C1] shadow transition hover:opacity-90"
            >
            ...
            {isAdmin && (
              <Link
                ...
                className="flex h-10 items-center gap-1.5 rounded-xl border border-amber-300 bg-amber-50 px-4 text-sm font-semibold text-amber-800 shadow-sm transition hover:bg-amber-100"
              >
