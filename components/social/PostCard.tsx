...
export default function PostCard({ post, onDeleted, onHide, compact = false }: PostCardProps) {
  ...
  const [reactions, setReactions] = useState<Record<string, { count: number; mine: boolean }>>(post.reactions || {});
      ...
      if (!post.reactions) {
        supabase
          ...
          .then(({ data: rows }) => {
            ...
            for (const r of (rows || []) as { emoji: string; user_id: string }[]) {
  ...
  const toggleReaction = async (emoji: string) => {
    ...
    setReactions((prev) => {
      const next = { ...prev };
      const entry = { count: next[emoji]?.count || 0, mine: !mine };
      if (mine) {
    ...
    if (mine) {
      await supabase.from('post_reactions').delete().eq('post_id', post.id).eq('user_id', myId).eq('emoji', emoji);
    } else {
      await supabase.from('post_reactions').insert({ post_id: post.id, user_id: myId, emoji });
    }
  };
            ...
            >
              ...
              {isOwner ? (
                <>
                  ...
                  <button onClick={() => { setMenuOpen(false); removePost(); }} className="flex w-full items-center gap-2 px-3 py-2.5 text-xs font-semibold text-red-500 hover:bg-red-50" role="menuitem">
