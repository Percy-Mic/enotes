...
CREATE TABLE public.report_events (
  ...
  action text NOT NULL CHECK (action = ANY (ARRAY['created'::text, 'reviewing'::text, 'resolved'::text, 'dismissed'::text, 'note'::text])),
  note text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
...
CREATE TABLE public.page_unlock_sessions (
  user_id uuid NOT NULL,
  page_id uuid NOT NULL,
  unlocked_until timestamp with time zone NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT page_unlock_sessions_pkey PRIMARY KEY (user_id, page_id),
  CONSTRAINT page_unlock_sessions_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT page_unlock_sessions_page_id_fkey FOREIGN KEY (page_id) REFERENCES public.journal_pages(id)
);