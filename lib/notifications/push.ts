/* ============================================================
   ...
   • Site closed: this module registers the browser's PushManager
     subscription; when a notifications row lands, the notify_push DB
     trigger posts to /api/push/send (see lib/notifications/send.ts),
     which delivers Web Push via the VAPID keys. The service worker
     (public/sw.js) shows the OS notification. No paid provider required.

   Env required:
   NEXT_PUBLIC_VAPID_PUBLIC_KEY — the VAPID *public* key (safe to
   expose; it is public by design). The PRIVATE key lives only in
   the server environment (VAPID_PRIVATE_KEY), never in client code.
   ============================================================ */
...
async function registerWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    ...
    /* `ready` resolves only once an ACTIVE worker serves this scope.
       Subscribing against a merely-registered worker throws
...
export async function enablePush(): Promise<string | null> {
  ...
  try {
    ...
    /* Browsers without a push backend (some Chromium builds) and offline
       machines both fail here — frame it as this browser/environment, not
       a bug in the user's account. */
    return `This browser couldn't enable push — ${detail}`;
  }
}
